...
describe "IrcProtocol" do
  ...
  it "should parse a language into an event" do
    ...
    event.should include(:rules)
    event[:rules].length.should eq(2)
    event[:rules][0].should include(:rule => :single)
    event[:rules][1].should include(:rule => :remaining)
  ...
  it "should parse optional chains" do
    event = @i.send(:parse_event, 'LIST [1 [1]]')[:rules][0]
  ...
  it "should parse csvlists" do
    event = @i.send(:parse_event, 'LIST *1')[:rules][0]
    event.should include(:rule => :csvlist)
  end
